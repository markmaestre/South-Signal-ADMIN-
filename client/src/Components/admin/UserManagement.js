...
import {
  ...
  Weight, Package, History, TrendingUp, AlertCircle, FileText, Camera
} from 'lucide-react';

// ── Design Tokens ─────────────────────────────────────────────────────────────
const C = {
  // Core palette — deep slate-navy system
  ink:        '#0D1829',
  inkMid:     '#1E2D47',
  inkLight:   '#2C4168',
  skyBlue:    '#2563EB',
...
const generateUserPDF = async (filteredUsers, analytics, adminRole, quickFilter, dateRange, barangayName) => {
  ...
  const drawPageHeader = () => {
    ...
    doc.setFontSize(8.5);
    doc.setFont('helvetica', 'bold');
    setRgb([147, 197, 253], 'text'); // blue-300
  ...
  // Build metric boxes based on role — southadmin never shows Central Bicutan, vice versa
  const demoMetrics = [
  ...
  // Superadmin gets both, already pushed above
  ...
  if (filteredUsers.length > 0) {
    ...
    // Build head and body based on role (drop Barangay column for single-barangay admins)
...
const UserManagement = ({ barangayFilter, adminRole }) => {
  ...
  const getAnalytics = () => {
    ...
    // Only compute the barangay count relevant to this admin's role
      ...
      <div style={S.analyticsBar}>
        ...
        <div style={S.analyticsCards}>
          {[
            ...
            // Superadmin sees both; each barangay admin sees only their own count
